...
modify column customer_id varchar(55);
...
create table  customer_Income_grade as select *, 
case
...
select * from customer_income_grade;

create table  customer_monthly_interest as select * ,
case 
...
select * from customer_monthly_interest;


-- loan 
create table loan_status
(Loan_ID varchar(55),
Customer_id varchar(55),Loan_Amount varchar(55),Loan_Amount_Term int,
Cibil_Score int);

select * from loan_status;

delimiter //
create trigger Loan_amount_Verify before insert on 
...
begin
 ...
 create table remark_update_detail(Loan_id	varchar(55),
Loan_amount	varchar(55),
Cibil_Score	int,
Cibil_Score_status varchar(55)
);

select * from remark_update_detail;


delimiter //
create trigger loan_cibil_score_Verify after insert on 
loan_status for each row
case 
when new.Cibil_score >900 then insert into remark_update_detail values(new.Loan_id,
new.Loan_amount	,new.Cibil_Score,"high cibil score");
when new.Cibil_score >750 then  insert into remark_update_detail values(new.Loan_id,
new.Loan_amount	,new.Cibil_Score,
"no penalty");
when new.Cibil_score >0 then  insert into remark_update_detail values(new.Loan_id,
new.Loan_amount	,new.Cibil_Score,
"penalty customers");
when new.Cibil_score <=0 then  insert into remark_update_detail values(new.Loan_id,
new.Loan_amount	,new.Cibil_Score,
...
create table  customer_loan as select c.loan_id,c.customer_id,c.applicantincome,c.coapplicantincome,c.property_area,c.loan_status,c.monthly_interest,r.loan_amount,
r.cibil_score,r.cibil_score_status from 
customer_monthly_interest c inner join remark_update_detail r on c.loan_id=r.loan_id;

select count(customer_id) from customer_loan;
...
delete from customer_loan
where cibil_score_status="reject customers" ;
...
update customer_det
set Gender = 'Female'
where Customer_id = 'IP43006';

update customer_det
set Gender = 'Female'
where Customer_id = 'IP43016';

update customer_det
set Gender = 'Male'
where Customer_id = 'IP43018';

update customer_det
set Gender = 'Male'
where Customer_id = 'IP43038';

update customer_det
set Gender = 'Female'
where Customer_id = 'IP43508';

update customer_det
set Gender = 'Female'
where Customer_id = 'IP43577';

update customer_det
set Gender = 'Female'
where Customer_id = 'IP43589';

update customer_det
set Gender = 'Female'
where Customer_id = 'IP43593';

update customer_det
set Age = 45
where Customer_id = 'IP43007';

update customer_det
set Age = 32
where Customer_id = 'IP43009';
...
create table customer_loan_analysis as select  cl.loan_id,cl.customer_id,cl.applicantincome,cl.coapplicantincome,cl.property_area,cl.loan_status,cl.loan_amount,cl.cibil_score,cl.cibil_score_status,
cd.gender,cd.age,cd.married,cd.education,cd.self_employed,cs.customer_name,cs.region_id,cs.postal_code,
cs.segment,cs.state,ri.region from customer_loan cl 
inner join customer_det cd on cl.customer_id=cd.customer_id
...
select * from customer_loan_analysis ;
select * from country_state;
select * from customer_det;
select * from region_info;
select * from customer_loan;
...
select max(cibil_score) from customer_loan_analysis ;
select * from customer_loan_analysis where segment in ("corporate","Home Office");

delimiter //
create procedure customer_loan_output()
begin
   select cl.*,cd.* FROM customer_loan cl 
LEFT JOIN customer_det cd on cl.customer_id=cd.customer_id
WHERE SELF_employed is null;

select cd.customer_id,cd.loan_id,cd.customer_name,cd.region_id, cd.gender,cd.education,cd.Age,cd.self_employed,cs.postal_code,cs.segment,cs.state from
customer_det cd  left join country_state cs on cd.customer_name=cs.customer_name where cd.self_employed is null;

select cs.customer_id,cs.loan_id,cs.customer_name,cs.region_id,cs.postal_code,cs.segment,cs.state,ri.region from 
country_state cs right join  region_info ri on  cs.region_id =ri.region_id where cs.region_id is null;

select max(cibil_score) from customer_loan_analysis ;-- output 3

select * from customer_loan_analysis where segment in ("corporate","Home Office");-- output 4
end //
delimiter ;


call customer_loan_output;


create table loan_cibil_score_status as select cla.* , cl.monthly_interest
  from customer_loan_analysis cla
inner join customer_loan cl on cla.customer_id=cl.customer_id;




 create table customer_interest_analysis as select cl.loan_id,cl.customer_id,cl.applicantincome,cl.coapplicantincome,cl.property_area,
 cl.loan_status,cl.loan_amount,cg.Grade,cl.cibil_score,cl.cibil_score_status,gender,age,married,education,self_employed,customer_name,region_id,postal_code,
 segment,state,region,
 CONCAT(monthly_interest, '%') AS interest_with_percentage, loan_amount*(monthly_interest/100) as interest_monthly,(loan_amount*monthly_interest/100)*12
 as Annual_intersest from loan_cibil_score_status cl 
 inner join customer_income_grade cg on cl.customer_id=cg.customer_id ;
select * from loan_cibil_score_status_final;
